...
// CController CRobot_ontrol; // NOTE: Create the object to calculate F.K.
...
void CRobot_Arm_TR::TX()
{
  ...
  //   for(int i = 1; i<2 ; i++)
  // {
  //   _profile_velocity[i] =  60000 * (_dxl_goal_position[i] - _dxl_present_position[i]) / (35 * 607500); // = 6,000,000 * (goal-present) / (profile_time2<3500ms> * resolution<607500>)
  //   _profile_acceleration[i] = 0.6 * _profile_velocity[i] ; // 0.6 = 600/Profile_t1<1000ms>    
  // }
  //   _profile_velocity[3] =  60000 * (_dxl_goal_position[3] - _dxl_present_position[3]) / (35 * 526374); // = 6,000,000 * (goal-present) / (profile_time2<3500ms> * resolution<526374>)
  //   _profile_acceleration[3] = 0.6 * _profile_velocity[3];   // 0.6 = 600/Profile_t1<1000ms>  

  //  _profile_velocity[4] =  6000 * (_dxl_goal_position[4] - _dxl_present_position[4]) / ( 3000 * 4096); // = 6,000,000 * (goal-present <cnt>) / (profile_time2<microsec> * resolution<4096>)
  //  _profile_acceleration[4] = 60 / 1 * _profile_velocity[4];   //_profile_acceleration = 60/t1(s)* _profile_velocity

    //time-based profile
    // for(int i = 1; i<4 ; i++)
    // {
    //   _profile_velocity[i] =  3000; //total time <ms>
    //   _profile_acceleration[i] = 1000; // accelelration time <ms>
    // }

  
    //time-based profile
    _profile_velocity[1] =  2400; //total time <ms>
    _profile_acceleration[1] = 800; // accelelration time <ms>
    _profile_velocity[2] =  2400; //total time <ms>
    _profile_acceleration[2] = 800; // accelelration time <ms>
    _profile_velocity[3] =  2400; //total time <ms>
    _profile_acceleration[3] = 800; // accelelration time <ms>
  
    /*
    //velocity-based profile
    _profile_velocity[1] =  6000 * (_dxl_goal_position[1] - _dxl_present_position[1]) / ( 3000 * 607500); // = 6,000,000 * (goal-present <cnt>) / (profile_time2<microsec> * resolution<607500>)
    _profile_acceleration[1] = 60 / 1 * _profile_velocity[4];   //_profile_acceleration = 60/t1(s)* _profile_velocity
    _profile_velocity[2] =  6000 * (_dxl_goal_position[2] - _dxl_present_position[2]) / ( 3000 * 607500); // = 6,000,000 * (goal-present <cnt>) / (profile_time2<microsec> * resolution<607500>)
    _profile_acceleration[2] = 60 / 1 * _profile_velocity[4];   //_profile_acceleration = 60/t1(s)* _profile_velocity
    _profile_velocity[3] =  6000 * (_dxl_goal_position[3] - _dxl_present_position[3]) / ( 3000 * 526374); // = 6,000,000 * (goal-present <cnt>) / (profile_time2<microsec> * resolution<526374>)
    _profile_acceleration[3] = 60 / 1 * _profile_velocity[4];   //_profile_acceleration = 60/t1(s)* _profile_velocity
    */
    _profile_velocity[4] =  6000 * (_dxl_goal_position[4] - _dxl_present_position[4]) / ( 3000 * 4096); // = 6,000,000 * (goal-present <cnt>) / (profile_time2<microsec> * resolution<4096>)
    _profile_acceleration[4] = 60 / 1 * _profile_velocity[4];   //_profile_acceleration = 60/t1(s)* _profile_velocity
...
void CRobot_Arm_TR::RX()
{
  ...
  // NOTE: uncommit below line to calculate the angle of the dynamixels
  // CRobot_ontrol.get_present_position(_dxl_present_position);
  // CRobot_ontrol.forwardKin(CRobot_ontrol._dxl_present_position[1], CRobot_ontrol._dxl_present_position[2], CRobot_ontrol._dxl_present_position[3]);
  // _x_e = CRobot_ontrol._x_e;
  // _y_e = CRobot_ontrol._y_e;
  // _alpha_e = CRobot_ontrol._alpha_e;
  // cout << "x:[" << _x_e << "]\ty:[" << _y_e << "]\talpha:[" << _alpha_e << "]\n";
}
